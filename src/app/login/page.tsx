...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError("Erro ao conectar com Google. Verifique se o provedor está ativo no Supabase.")
      console.error(err)
      setLoading(false)
    }
  }
      ...
      <Card className="w-full max-w-md bg-card/60 backdrop-blur-3xl border-border shadow-2xl rounded-[2.5rem] overflow-hidden animate-in zoom-in duration-700">
        <CardHeader className="pt-12 pb-6 text-center space-y-6">
          ...
          <div className="space-y-2">
            <CardTitle className="text-xl font-bold uppercase tracking-widest text-primary/80">Painel de Gestão</CardTitle>
        ...
        <CardContent className="px-8 pb-12 space-y-8">
          <div className="bg-primary/5 border border-primary/20 p-4 rounded-2xl text-center">
            ...
            <Button 
              ...
              disabled={loading}
              className="w-full h-16 rounded-3xl bg-white text-black hover:bg-gray-100 font-bold text-lg gap-3 shadow-xl transition-all active:scale-95 border border-gray-200"
            >
